...
import { Star, TrendingUp, Plus, X, Edit2, Trash2, Loader2 } from "lucide-react";
...
export default function Skills() {
  const { skills, addSkill, deleteSkill, updateSkill, loadingData } = useDev();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await updateSkill(editId, { ...form, level: Number(form.level) });
        setEditId(null);
      } else {
        await addSkill({ ...form, level: Number(form.level) });
      }
      setForm(EMPTY_FORM);
      setShowForm(false);
    } catch (err) {
  ...
  return (
    <Layout title="Compétences">
      ...
      <div className="grid grid-cols-3 gap-4 mb-7">
        <StatCard label="Total"          value={totalSkills}    icon={Star}       color="indigo" />
        <StatCard label="Niveau moyen"   value={`${avgLevel}%`} icon={TrendingUp}  color="purple" />
        <StatCard label="Niveau expert"  value={expertSkills}   icon={Star}       color="green"  sub="≥ 80%" />
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl p-6 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-bold text-gray-800 dark:text-white">
              {editId ? "Modifier la compétence" : "Nouvelle compétence"}
            </h3>
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid sm:grid-cols-3 gap-4 mb-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Nom</label>
                <input
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Niveau (%)</label>
                <input
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Catégorie</label>
                <select
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={cancelForm}
                className="text-sm font-medium text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-white transition-colors px-4 py-2.5"
              >
                Annuler
              </button>
            </div>
      ...
      {loadingData ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400 dark:text-slate-500">
          ...
          <p className="text-sm font-medium">
            {filter !== "All" ? `Aucune compétence "${filter}".` : "Aucune compétence. Ajoutez-en une !"}
          </p>
        </div>
      ) : (
          ...
          {filtered.map((s) => (
            ...
            >
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                <button
                  onClick={() => startEdit(s)}
                  className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-gray-500 dark:text-slate-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 hover:text-indigo-600 transition-colors"
                >
                  <Edit2 size={13} />
                </button>
                <button
                  onClick={() => handleDelete(s.id)}
                  className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-gray-500 dark:text-slate-400 hover:bg-red-100 dark:hover:bg-red-900/30 hover:text-red-500 transition-colors"
                >
                  <Trash2 size={13} />
                </button>
              </div>
